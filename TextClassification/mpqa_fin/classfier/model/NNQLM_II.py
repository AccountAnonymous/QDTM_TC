...
class NNQLM_II(object):
    ...
    def add_embeddings(self):
        ...
        self.embedded_chars_q = self.reduc_matrix(self.embedded_chars_q,self.weighted_q)
    ...
    def reduc_matrix(self, sentence_matrix, sentence_weighted):
        ...
        self.norm_reduced = tf.nn.l2_normalize(sentence_matrix,2) #10 22 50 1 [batch left emb 1]
        
        reverse_matrix = tf.transpose(self.norm_reduced, perm = [0,1,3,2]) # 10 22 1 50

        q_a = tf.matmul(self.norm_reduced,reverse_matrix)# 10 22 50 50

        for i in range(self.max_input_left-1):
