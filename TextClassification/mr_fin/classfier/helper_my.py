    ...
    if is_embedding_needed:
        ...
        if isEnglish:
            if dim == 50:
                ...
                fname = "/sdb/GH/classify/embedding/glove.6B.50d.txt"
                #fname = "/sdb/GH/classify/embedding/glove.27B.50d.txt"
...
def load_ag_news(dataset=dataset):
    data_dir = "../data/" + dataset
    datas = []
    if dataset=='ag_news':
        for data_name in ['train.csv', 'dev.csv']:
            if data_name == 'train.csv':
                data_file = os.path.join(data_dir, data_name)
                data = pd.read_csv(data_file, header=None, usecols=[0,2], sep=",", names=[
                                   "flag", "question"], quoting=1).fillna("WASHINGTON")
...
def batch_gen_with_single(df, alphabet, batch_size=10, q_len=33):
    ...
    for index, row in df.iterrows():
        ...
        pairs.append((quetion, q_position, q_lens))
...
def batch_gen_with_point_wise(df, alphabet, batch_size=10, q_len=33):
    ...
    for index, row in df.iterrows():
        ...
        pairs.append((question, label, q_position, q_lens))
    n_batches = int(len(pairs) * 1.0 / batch_size)
