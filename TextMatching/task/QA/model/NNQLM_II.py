...
class NNQLM_II(object):
    ...
    def create_placeholder(self):
        ...
        self.a_lens = tf.placeholder(tf.int32,[self.batch_size],name = 'answer_length')
    ...
    def density_weighted(self):
        ...
        self.weighted_q = tf.Variable(tf.ones([1,self.max_input_left + (self.max_input_left - 2)*2,1,1]), name = 'weighted_q')
        self.para.append(self.weighted_q)
        self.weighted_a = tf.Variable(tf.ones([1,self.max_input_right + (self.max_input_right - 2)*2,1,1]), name = 'weighted_a')
    ...
    def add_embeddings(self):
        ...
        self.embedded_chars_q = self.concat_embedding(self.question,self.q_overlap,self.q_position,self.q_lens)
        self.embedded_chars_a = self.concat_embedding(self.answer,self.a_overlap,self.a_position,self.a_lens)
        # reduce_matrix
    ...
    def joint_representation(self):
        ...
        self.density_q = self.reduc_matrix(self.embedded_chars_q,self.weighted_q, self.max_input_left)

        self.density_a = self.reduc_matrix(self.embedded_chars_a,self.weighted_a, self.max_input_right)
        ...
        # self.a_trace = self.trace_represent(self.reduced_matrix)
    ...
    def reduc_matrix(self, sentence_matrix, sentence_weighted, sentence_lens):
        ...
        for i in range(sentence_lens-2):

            # tmp = tf.expand_dims(tf.matmul(self.norm_reduced[:,i,:,:], reverse_matrix[:,i+1,:,:]), 1)
            tmp = tf.matmul(tf.tile(tf.expand_dims(self.norm_reduced[:,i,:,:], 1), [1,2,1,1]), reverse_matrix[:,i:i+2,:,:])
    ...
    def feed_neural_work(self):     
        with tf.name_scope('regression'):
            ...
            W = tf.Variable(tf.zeros(shape = [7742,2]),name = 'W')
    ...
    def concat_embedding(self,words_indice,overlap_indice,position_indice,lens):
